# HANDOFF — Histiva Marketing Site (Implementation Session)
...
You are about to execute a **35-task implementation plan** for `histiva.github.io` — the public marketing website for **Histiva**, the nonprofit hospital information system. Brainstorming and planning are done. The spec is approved. The plan is approved. Your job is to ship it task-by-task using **subagent-driven development**.

Two committed files contain everything you need:

1. **Spec** (authoritative on intent): `docs/superpowers/specs/2026-05-13-histiva-marketing-site-design.md` — 18 sections.
2. **Plan** (authoritative on steps): `docs/superpowers/plans/2026-05-13-histiva-marketing-site.md` — 35 tasks across 5 phases (A → E).
...
**IS:**
- A static marketing site for Histiva, the nonprofit hospital information system (HBYS).
- Deployed to GitHub Pages at `https://histiva.github.io/`.
...
**ISN'T:**
- The HBYS product itself — that lives at `/home/cevheri/projects/hbys/histiva-app/` (a separate Next.js + PostgreSQL repo, do NOT modify it from this session).
...
| **Brand** | Trust Blue palette (`#0c2461` / `#1e40af` / `#3b82f6` / `#dbeafe`), Inter variable for everything, wordmark logo "Histiva" for v1. |
...
| **Domain** | `histiva.github.io` for v1. Custom domain is a Phase-2 decision (CNAME slot reserved in `public/`). |
...
1. `docs/superpowers/specs/2026-05-13-histiva-marketing-site-design.md` (15-min read)
2. `docs/superpowers/plans/2026-05-13-histiva-marketing-site.md` (30-min read)
...
1. **Read the task block** in the plan (`docs/superpowers/plans/2026-05-13-histiva-marketing-site.md`).
...
```
origin  https://github.com/histiva/histiva.github.io.git
```
...
- **Cookie banner gates GA4 strictly.** The `<Analytics />` component checks `localStorage.getItem('histiva.consent') === 'accept'` before loading gtag. Listens for `histiva:consent-accepted` event after the user accepts. Test in a clean profile that no GA request fires on first load.
...
1. Confirm you're in `/home/cevheri/projects/hbys/histiva.github.io` (run `pwd`).
...
The user's auto-memory (`~/.claude/projects/.../histiva-app/memory/`) is keyed to the **histiva-app** repo, not this one. **In this fresh session in `histiva.github.io`, your memory dir will be different and start empty.** Don't expect prior context to load. This HANDOFF.md is the bridge.
...
- All 8 pages render at https://histiva.github.io/
